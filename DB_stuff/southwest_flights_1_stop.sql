CREATE VIEW `southwest_flights_1_stop` AS
    SELECT 
        f1.Id_southwest_flight AS f1_id,
        f2.Id_southwest_flight AS f2_id,
        NULL AS f3_id,
        '1-stop' AS flight_type,
        1 AS num_of_stopovers,
        CONCAT(IFNULL(a1.local_code, a1.iata_code),
                CHAR(58),
                IFNULL(a2.local_code, a2.iata_code),
                CHAR(58),
                IFNULL(a3.local_code, a3.iata_code)) AS route,
        f1.flight_number AS f1_flight_number,
        TIME_FORMAT(TIMEDIFF(f2.arrive_date_time, f1.depart_date_time),
        ...
        TIME_FORMAT(TIMEDIFF(f2.depart_date_time, f1.arrive_date_time),
                '%hhr %imn') AS stopover_duration,
        TIME_FORMAT(TIMEDIFF(f2.arrive_date_time, f2.depart_date_time),
                '%hhr %imn') AS f2_duration,
        f1.flight_cost_coach + f2.flight_cost_coach AS trip_total_cost_coach,
        f1.flight_cost_business + f2.flight_cost_business AS trip_total_cost_business,
        ...
        f2.arrive_airport AS f2_destination,
        a3.airport_type AS f2_destination_type,
        a3.airport_name AS f2_destination_name,
        a3.latitude_deg AS f2_destination_latitude,
        a3.longitude_deg AS f2_destination_longitude,
        a3.region_name AS f2_destination_region,
        a3.local_region AS f2_destination_local_region,
        a3.municipality AS f2_destination_municipality,
        a3.iata_code AS f2_destination_iata,
        a3.local_code AS f2_destination_local_code
    FROM
        atr.southwest_flights AS f1
            INNER JOIN
        atr.southwest_flights AS f2 ON f1.arrive_airport = f2.depart_airport
            INNER JOIN
